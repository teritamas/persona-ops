...
const mockProjects = [
  {
    ...
    isInitial: false,
    personas: [
      { id: 'p1', name: '鈴木 健太', role: '現場セールス', x: 20, y: 30, avatarSeed: 'Felix', traits: ['効率重視', '外出多い'], reaction: { type: 'positive', text: '移動中の入力が楽になりそう！' } },
      { id: 'p2', name: '佐藤 真由美', role: 'マネージャー', x: 70, y: 40, avatarSeed: 'Aneka', traits: ['データ重視', '管理職'], reaction: { type: 'neutral', text: '要約の精度が気になります。' } },
      { id: 'p3', name: '田中 宏', role: '内勤営業', x: 45, y: 60, avatarSeed: 'Jasper', traits: ['PCメイン', '丁寧'], reaction: { type: 'negative', text: '社内だと声に出しづらいかも…' } },
      { id: 'p4', name: '高橋 涼子', role: '営業企画', x: 80, y: 70, avatarSeed: 'Avery', traits: ['分析好き', '新しい物好き'], reaction: { type: 'positive', text: 'データ化が早まるのは大賛成です。' } },
      { id: 'p5', name: '伊藤 健', role: '若手セールス', x: 30, y: 80, avatarSeed: 'Leo', traits: ['スマホネイティブ', 'フットワーク軽'], reaction: { type: 'positive', text: 'タイピングより早くて最高です！' } },
    ],
    chats: [
      {
        id: 'chat_1',
...
function getActiveProject() {
  const proj = mockProjects.find(p => p.id === state.activeProjectId) || mockProjects[0];
  proj.activeChat = proj.chats && proj.chats.find(c => c.id === proj.activeChatId) || (proj.chats && proj.chats[0]) || { messages: [] };
  return proj;
}
