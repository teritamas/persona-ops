...
左側ナビゲーションの「シミュレーション」から、現在のプロジェクトに紐づく実行履歴を確認できる。チャット上でペルソナ作成、要件ドラフト保存、シミュレーション開始、要件承認が完了した場合は、完了イベントに応じてシミュレーション広場を更新する。実行中のシミュレーションはロングポーリングで進捗を追跡し、過去の履歴を選択すると、承認時点の要件、集計スコア、各ペルソナの反応を表示する。
